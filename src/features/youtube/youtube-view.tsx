...
const TOOLS = [
  {
    id: "package",
    label: "Video Package",
    desc: "Titles + Script + Description in one job.",
    icon: Layers,
...
export function YoutubeView() {
  ...
  // Video Package — runs 3 tools in sequence + auto-saves as a Document
  const packageM = useMutation({
    mutationFn: () =>
      api<{
        results: { titles: string; script: string; description: string };
        documentId: string;
        credits: number;
      }>("/api/youtube/package", {
        method: "POST",
    ...
    onSuccess: (data) => {
      ...
      qc.invalidateQueries({ queryKey: ["generator-history", "youtube"] });
      toast.success(`Video Package complete — saved to Documents · ${data.credits} credits left`);
    },
    onError: (e: Error) => toast.error(e.message),
  });
          ...
          <Card className="p-4 sm:p-5 md:p-6">
            ...
            {isPackage && (
              <div className="mb-4 flex items-start gap-2.5 rounded-lg border border-fuchsia-500/30 bg-fuchsia-500/5 px-3.5 py-2.5">
                ...
                <p className="text-xs text-fuchsia-700 dark:text-fuchsia-400">
                  <span className="font-medium">Video Package runs 3 tools in sequence</span> —
                  Titles → Script → SEO Description. Costs 3x credits. The combined
                  result is auto-saved to Documents so you have a complete video kit in one place.
                </p>
              </div>
            )}
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              >
                ...
                {isPackage ? "Run Video Package" : "Generate"}
              </Button>
              {result && (
